...
class StatProjetController extends Controller {

    public function statprojetAction() {
        // Chart
        $ob = new Highchart();
        $ob->chart->renderTo('linechartP');
        $ob->title->text('Pourcentage des projets ajoutés par mois');
        ...
        $query=$em->createQuery('SELECT count(p.id) as nombre, MONTH(p.dateAjoutProjet) mois FROM VworkitBundle:Projet p GROUP BY mois');
        ...
        return $this->render('VworkitBundle:Stat:statprojet.html.twig', array(
                    'chart2' => $ob
        ));
    }
